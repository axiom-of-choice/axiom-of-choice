...
| [mlx-community](https://huggingface.co/mlx-community?search=nemotron-parse) | Quantization | MLX quantizations for Apple Silicon, including the first port of the Nemotron-Parse OCR architecture, and an 8-bit GLM-4.1V-9B-Thinking that preserves the vision path, each evaluated against the bf16 source on fidelity and performance. |
